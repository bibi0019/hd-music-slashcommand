    ...
    run: async (interaction, client) => {
        ...
        if (!queue) {
            const queueError = new Discord.MessageEmbed()
                .setDescription("<:kho_hieu:987563858682085427> There is Nothing Playing")
                .setColor("RANDOM")
        ...
        try {
            ...
            await interaction.reply("***Paused the current track*** <:ok_bro:987562332970442792>")
            const message = await interaction.fetchReply()
            await message.react("⏸")
        } catch {
            interaction.reply({ content: "<:kho_hieu:987563858682085427> The queue has been paused already ", ephemeral: true })
        }
    }
}
